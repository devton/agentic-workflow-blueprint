...
- `constraints`
- `operationalContext` (environment tier, maintenance window, blast radius, rollback strategy)
- `complianceContext` (security baseline, audit obligations, change-ticket references)
...
3. Confirm proposed `workflowsWanted` mapping (plan section -> workflow id).
   - Ensure infra plans map to explicit workflows (`network-engineering`, `infra-operations`, `iac`, `os-platform`) when applicable.
...
7. Validate operational readiness annotations for infra-heavy plans:
   - change window documented;
   - rollback command path documented;
   - post-change verification commands documented.
...
- Infra plans include rollback and post-change verification notes.
